...
//检测如果“=”右边全部都是half，则删掉__half2float()
...
public:
  ...
  bool VisitBinaryOperator(BinaryOperator *binaryOp) {
    ...
    // 定义一个flag，
    // 0表示全half，没有double等,需要去掉__half2float()
    // 1表示检测到非Half的类型，不做处理

      // int flag1=0;
      // find_DeclRefExpr(left,SM,loc,flag1);
      // if (flag1==0) {
      //   // printf("这个=的flag为0\n");
      //   handle_half(left);
      //   // printf("去掉__halft2float()\n");
      // }
      // else {
      //   // printf("这个=的flag为1\n");
      // }
      ...
      int flag2=0;
      // printf("处理右节点\n");
      find_DeclRefExpr(right,SM,loc,flag2);
      if (flag2==0) {
        // printf("这个=的flag为0\n");
        handle_half(right);
        // printf("去掉__halft2float()\n");
...
//查找=右边部分的DeclRefExpr节点，有没有非half类型的变量
  ...
  //获取前面若干字符的位置
  SourceLocation getPrevCharsLocation(SourceLocation loc,SourceManager &SM,unsigned offset) {
    if (loc.isInvalid()) return loc;
    //分解为(文件ID，文件内偏移量)
    std::pair<FileID,unsigned> decomposed=SM.getDecomposedLoc(loc);
    if (decomposed.second<offset) {
      return SourceLocation();
    }
    return SM.getComposedLoc(decomposed.first,decomposed.second-offset);
    
  }

  //获取后面若干字符的位置
  SourceLocation getNextCharsLocation(SourceLocation loc, SourceManager &SM, unsigned offset) {
      if (loc.isInvalid()) return SourceLocation();
      
      std::pair<FileID, unsigned> decomposed = SM.getDecomposedLoc(loc);
      unsigned newOffset = decomposed.second + offset;

      // 检查是否超出文件大小
      if (const FileEntry* file = SM.getFileEntryForID(decomposed.first)) {
          if (newOffset > file->getSize()) {
              return SourceLocation(); // 越界保护
          }
      }
      
      return SM.getComposedLoc(decomposed.first, newOffset);
  }

  bool handle_half1(Stmt *left) {
    if (isa<DeclRefExpr>(left)) {
      ...
      if (ValueDecl *decl = declRefExpr->getDecl()) {
          ...
          if (typeStr=="struct __half"||typeStr=="__half"||typeStr=="half") {
            llvm::outs() << "发现half节点 " ;
                //添加处理逻辑
                // 获取变量名和源码范围
                ...
                //获取字符位置
                SourceManager &SM = TheRewriter.getSourceMgr();
                SourceLocation prev13Loc=getPrevCharsLocation(startLoc,SM,13);
                SourceLocation next1Loc=getNextCharsLocation(endLoc,SM,1);

                // 构建替换表达式
                string replacement =  varName ;

                TheRewriter.ReplaceText(SourceRange(prev13Loc, next1Loc), replacement);
                
                // llvm::outs() << "已去掉变量：" << varName 
                //             << "的__half2float()" << "\n";
          }
      }
      // else llvm::outs() <<"获取变量声明失败"<<"\n";
    ...
    // 递归访问当前语句的所有子语句
      ...
      llvm::outs() <<"不是DeclRefExpr节点"<<"\n";
      for (Stmt::child_iterator ci = left->child_begin(), ce = left->child_end(); ci != ce; ++ci) {
        cout<<"进入一层递归"<<endl;
        handle_half(*ci);
        cout<<"离开一层递归"<<endl;
      }
