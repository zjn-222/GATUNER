...
int main() {
  ...
  auto start = std::chrono::high_resolution_clock::now();
    ...
    for (uint_t i = 0; i < N; i++) {
        // scale i to [0,2*pi)
        float x = ((2.0*PI) / (double)N) * (double)i;

        // minor variation on a square wave (the classic FFT encoding example)
        in_real[i] = sgn(sin(x+1.0))+cosf(x);
        in_imag[i] = sgn(cosf(x+1.0))+sin(x);
    }
  ...
  inFile.close();
  // 更新当前染色体的数据
  data["chromosome100"]["runtime"] = duration.count(); // 此处键名会被 sed 动态替换
  data["chromosome100"]["error"] = program_error;
  // 写回文件
