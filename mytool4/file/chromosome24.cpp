...
int main() {
  ...
  auto start = std::chrono::high_resolution_clock::now();
    ...
    for (uint_t i = 0; i < N; i++) {
        ...
        in_real[i] = sgn(sinf(x+1.0))+cos(x);
        in_imag[i] = sgn(cos(x+1.0))+sinf(x);
    }
  ...
  inFile.close();
  // 更新当前染色体的数据
  data["chromosome24"]["runtime"] = duration.count(); // 此处键名会被 sed 动态替换
  data["chromosome24"]["error"] = program_error;
  // 写回文件
