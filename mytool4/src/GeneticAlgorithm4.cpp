...
private:
    ...
    const double mutationRate = 0.9;  // 变异概率
    const double crossoverRate = 0.9;  // 交叉概率
...
public:
    ...
    GeneticAlgorithm() {
        ...
        // 打开日志文件
        logFile.open("/home/zjn/mytool4/log/auto_test_simpsons_e10_mu09_cs09.txt", std::ios::out);
        if (!logFile.is_open()) {
...
private:
    ...
    std::vector<Individual> initializePopulation() {
        ...
        population[0].chromosome={1,1,1,1,1,1,2,2,2,2};
